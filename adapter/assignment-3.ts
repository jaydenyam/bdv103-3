    ...
    filters.some(
      (filter) =>
        ...
        (filter.genre === undefined ||
          book.genre?.toLowerCase() === filter.genre.toLowerCase()) &&
        (filter.publicationDateFrom === undefined ||
          new Date(book.publicationDate || '') >=
            new Date(filter.publicationDateFrom)) &&
        (filter.publicationDateTo === undefined ||
          new Date(book.publicationDate || '') <=
            new Date(filter.publicationDateTo))
    )
  );
}
